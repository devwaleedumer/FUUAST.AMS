...
{
    public class LocalFileStorageService(IOptions<OriginOptions> originSettings, IWebHostEnvironment _env) : ILocalFileStorageService
    {
        public async Task<string> UploadAsync<T>(FileRequest? request, FileType supportedFileType, CancellationToken cancellationToken = default)
            where T : class
        {
            ...
            if (streamData.Length > 0)
            {
                ...
                fileName += string.Concat(".", request.Extension.Trim());
                ...
                var imageUri = string.Concat(originSettings.Value.OriginUrl!, path);
                return imageUri;
            }
            else
            {
